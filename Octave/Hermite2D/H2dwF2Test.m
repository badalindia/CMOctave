...
fxx = @(x,y) -C1*C1.*sin(C1.*x).*sin(C2.*y);
fyy = @(x,y) -C2*C2.*sin(C1.*x).*sin(C2.*y);
...
for i=[1:I]
	...
	[fh fhx fhy fhxy fhxx fhyy] = H2dwF2(XS, YS, fS, fxS, fyS, fxyS, XR, YR);
	fyyExact = fyy(XR, YR);	
	
	e(i) = max(max(abs(fhyy - fyyExact)));
	d(i) = h;
