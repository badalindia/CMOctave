...
%step 1, calculating U*
	...
	ConvY_ = fft2(ConvY([1:Nc], [1:Nc]));
		...
		_x0 = H2dwm(Xc, Yc, Hc, Hxc, Hyc, Hxyc, Xf, Yf);
		_y0 = H2dwm(Xc, Yc, Vc, Vxc, Vyc, Vxyc, Xf, Yf);
		
		_x = H2dwm(Xf, Yf, Hf, Hxf, Hyf, Hxyf, _x0, _y0);
		_y = H2dwm(Xf, Yf, Vf, Vxf, Vyf, Vxyf, _x0, _y0);
		
		_rho = bilinearInterpolation(Xs, Ys, _x, _y, rho0);
		_t = bilinearInterpolation(Xs, Ys, _x, _y, t0);
		
		Fb  = -alpha*_rho + beta*_t;
...
%advection of chi
	...
	H0_xpyp  = H2dwm(Xc, Yc, Hc, Hxc, Hyc, Hxyc, x0_xpyp, y0_xpyp);
	H0_xpyn  = H2dwm(Xc, Yc, Hc, Hxc, Hyc, Hxyc, x0_xpyn, y0_xpyn);
	H0_xnyp  = H2dwm(Xc, Yc, Hc, Hxc, Hyc, Hxyc, x0_xnyp, y0_xnyp);
	H0_xnyn  = H2dwm(Xc, Yc, Hc, Hxc, Hyc, Hxyc, x0_xnyn, y0_xnyn);
	
	V0_xpyp  = H2dwm(Xc, Yc, Vc, Vxc, Vyc, Vxyc, x0_xpyp, y0_xpyp);
	V0_xpyn  = H2dwm(Xc, Yc, Vc, Vxc, Vyc, Vxyc, x0_xpyn, y0_xpyn);
	V0_xnyp  = H2dwm(Xc, Yc, Vc, Vxc, Vyc, Vxyc, x0_xnyp, y0_xnyp);
	V0_xnyn  = H2dwm(Xc, Yc, Vc, Vxc, Vyc, Vxyc, x0_xnyn, y0_xnyn);
	...
	t
			...
			%resetting initial conditions and diffeo
			if(mod(ctr, gridShiftN*100) == 0)
			
				 %using sampling grid
				__x = H2dwm(Xf, Yf, Hf, Hxf, Hyf, Hxyf, Xs, Ys);
				__y = H2dwm(Xf, Yf, Vf, Vxf, Vyf, Vxyf, Xs, Ys);
				
				rho0 = bilinearInterpolation(Xs, Ys, __x, __y, rho0);
				t0 = bilinearInterpolation(Xs, Ys, __x, __y, t0);
...
if(mod(ctr, plotCtr)==0)
        ...
        t
                ...
                G1 = 2-bilinearInterpolation(Xs, Ys, __x, __y, rho0);
                G2 = bilinearInterpolation(Xs, Ys, __x, __y, t0);
                
                max(max(G2))
        
			%printing diffeo
			#{
			plot(Xs(Np/2, :), __x(Np/2, :));
			print( strcat('Images/', target, '/d1/d1_', num2str(ctr2),'.png'), Res);
			#}
			
				%vertical lines
				_xv = H2dwm(Xf, Yf, Hf, Hxf, Hyf, Hxyf, Xs(:, [Np/Nf:Np/Nf:Np]), Ys(:, [Np/Nf:Np/Nf:Np]));
				_yv = H2dwm(Xf, Yf, Vf, Vxf, Vyf, Vxyf, Xs(:, [Np/Nf:Np/Nf:Np]), Ys(:, [Np/Nf:Np/Nf:Np]));
				
				%horizontal lines
				_xh = H2dwm(Xf, Yf, Hf, Hxf, Hyf, Hxyf, Xs([Np/Nf:Np/Nf:Np], :), Ys([Np/Nf:Np/Nf:Np], :));
				_yh = H2dwm(Xf, Yf, Vf, Vxf, Vyf, Vxyf, Xs([Np/Nf:Np/Nf:Np], :), Ys([Np/Nf:Np/Nf:Np], :));
				
				plot(_xv, _yv, '-r');
				hold on;
				plot(_xh', _yh', '-r');
				hold off;
				print( strcat('Images/', target, '/d2/d2_', num2str(ctr2),'.png'), '-r400');
        ...
        %myquiver(2, Xc, Yc, U0, V0, 
        %strcat('Images/', target, '/v/vel_',num2str(ctr2),'.png'));
        
        %myplot(3, Xc, Yc, U0xy, nC, jet, 0, 
        %strcat('Images/', target, '/uxy/uxy_',num2str(ctr2),'.png'), Res);
        
        %myplot(4, Xc, Yc, V0xy, nC, jet, 0, 
        %strcat('Images/', target, '/vxy/vxy_',num2str(ctr2),'.png'), Res);
